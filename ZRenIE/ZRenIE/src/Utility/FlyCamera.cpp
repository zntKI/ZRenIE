...
FlyCamera::FlyCamera(glm::vec3 position, glm::vec3 up, float yaw, float pitch)
	: Front(glm::vec3(0.0f, 0.0f, -1.0f)), MovementSpeed(SPEED), MovementSpeedFast(SPEED_FAST), currentMovementSpeed(SPEED), MouseSensitivity(SENSITIVITY), Zoom(ZOOM)
{
	Position = position;
	WorldUp = up;
	Yaw = yaw;
...
void FlyCamera::ProcessKeyboard(FlyCamera_Movement direction, float deltaTime)
{
	if (direction == MOVE_FAST)
		currentMovementSpeed = MovementSpeedFast;
	else if (direction == SLOW_DOWN)
		currentMovementSpeed = MovementSpeed;

	float velocity = currentMovementSpeed * deltaTime;
